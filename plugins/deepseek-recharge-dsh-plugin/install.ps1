# Install the deepseek_recharge tool into a native dsh installation.
#
# Usage:  powershell -ExecutionPolicy Bypass -File .\install.ps1
#
# Steps:
#   1. Locate $DSH_HOME (defaults to ~/.dsh) and the web profile.
#   2. Copy plugins/deepseek-recharge into the profile.
...
$pluginDir = Join-Path $profileDir 'plugins\deepseek-recharge'
...
Copy-Item -Path (Join-Path $PSScriptRoot 'plugins\deepseek-recharge\index.js') -Destination $pluginDir -Force
Copy-Item -Path (Join-Path $PSScriptRoot 'plugins\deepseek-recharge\package.json') -Destination $pluginDir -Force
Write-Host "OK  plugin copied -> $pluginDir" -ForegroundColor Green

# --- 2. Add the profile patch entry (idempotent) ---------------------------
$entryText = @'

# --- deepseek_recharge tool (native dsh) ---
# DeepSeek has no recharge API; this tool shows the current balance and opens
# the platform recharge page in the default browser (configurable autoOpen).
- insert:
...
if ($current -match 'tool-deepseek-recharge') {
...
Write-Host '  3. Ask: "DeepSeek 余额不多了，帮我打开充值页"'
