...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
      ...
      render: (_args, value) => [{
        ...
        text: value.positions.length === 0 ? '(无匹配建议)'
          : value.positions.map((p) =>
            `[${p.status}] ${p.date} ${p.name}(${p.symbol}) ${p.action}(${p.kind ?? 'order'}/${p.phase ?? '-'}) 建议价=${p.advicePrice} `
            + `目标=${p.targetPrice ?? '-'} 止损=${p.stopLoss ?? '-'} 仓位=${p.positionPct}% `
    ...
    async execute(args) {
      ...
      const normalize = (p) => ({
        ...
        positionPct: p.positionPct, reason: p.reason ?? null, sentimentId: p.sentimentId ?? null,
      ...
      return { positions: positions.slice().reverse().map(normalize), pendingTotalPct }
    },
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'T+1 验单结算（建议即挂单）：对每条 pending 的 order 挂单建议（kind=order），'
      ...
      + '（buy 按总资产×仓位比例买入、sell 卖出全部持仓），建议置 executed；落在区间外 → 挂单作废（cancelled）'
      + '并记录偏离原因。kind=prediction（预测观察）不参与验单。preview=true 只试算不落库。'
      + '已结算（settledAt）的建议不会重复处理。返回每条建议的判定明细。',
    parameters: {
    ...
    async execute(args) {
      ...
      for (const position of pending) {
        ...
        if (position.action === 'buy') {
          ...
          if (shares <= 0) {
            skipped += 1
            base.reason = `预算不足一手：预算 ${round2(budget)} 元 @ ${price}，保持挂单`
            results.push(base)
            continue
          }
          ...
          if (shares <= 0) {
            skipped += 1
            base.reason = `模拟盘无 ${symbol} 持仓可卖，保持挂单`
            results.push(base)
            continue
          }
          amount = roundMoney(shares * price)
