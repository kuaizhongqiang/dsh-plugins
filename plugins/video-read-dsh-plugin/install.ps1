# Install the read_video tool into a native dsh installation.
#
# Usage:  powershell -ExecutionPolicy Bypass -File .\install.ps1
#
# Steps:
#   1. Locate $DSH_HOME (defaults to ~/.dsh) and the web profile.
#   2. Copy plugins/video-read into the profile.
...
$pluginDir = Join-Path $profileDir 'plugins\video-read'
...
Copy-Item -Path (Join-Path $PSScriptRoot 'plugins\video-read\index.js') -Destination $pluginDir -Force
Copy-Item -Path (Join-Path $PSScriptRoot 'plugins\video-read\package.json') -Destination $pluginDir -Force
Write-Host "OK  plugin copied -> $pluginDir" -ForegroundColor Green

# --- 2. Add the profile patch entry (idempotent) ---------------------------
$entryText = @'

# --- read_video tool (native dsh) ---
# The main model stays text-only; it calls read_video to understand a video
# (local path or public URL) through the Xiaomi MiMo full-modal model.
- insert:
    - id: tool-video-read
      name: './plugins/video-read/index.js'
'@
$current = if (Test-Path $patchFile) { Get-Content $patchFile -Raw } else { '' }
if ($current -match 'tool-video-read') {
...
Write-Host '  3. Ask the model to read a video: give it a local path or a public URL.'
